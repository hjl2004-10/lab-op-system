...
export function useAppState() {
  ...
  const [studentProfiles, setStudentProfiles] = useLocalStorage<StudentProfile[]>(
    "gantt-final-data",
    [],
    "studentProfiles"
  );

  // Set default passwords on first load
  // Priority: Person.passwordHash (syncable) > localStorage (backward compatible)
  useEffect(() => {
    let hasChanges = false;
    const updatedPeople = people.map((p) => {
      // If person already has passwordHash in AppState, also sync to localStorage
      if (p.passwordHash) {
        localStorage.setItem(`gantt-pwd-${p.id}`, p.passwordHash);
        return p;
      }
      // No passwordHash in AppState - check localStorage or set default
      const stored = localStorage.getItem(`gantt-pwd-${p.id}`);
      if (stored) {
        // localStorage has it - sync back to AppState
        hasChanges = true;
        return { ...p, passwordHash: stored };
      }
      // No password anywhere - set default
      hasChanges = true;
      const defaultPwd = p.role === "admin" ? "BTBU2023" : "000";
      const hashed = simpleHash(defaultPwd);
      localStorage.setItem(`gantt-pwd-${p.id}`, hashed);
      return { ...p, passwordHash: hashed };
    });
    if (hasChanges) {
  ...
  const addPerson = useCallback(
    (name: string) => {
      ...
      storePasswordHash(newPerson.id, defaultPassword);
  ...
  // Update person properties (e.g., passwordHash for cross-device sync)
  ...
  const setPersonPassword = useCallback(
    (personId: string, password: string) => {
      // Update both localStorage (backward compat) and AppState (syncable)
      storePasswordHash(personId, password);
      setPeople((prev) =>
        prev.map((p) =>
          p.id === personId ? { ...p, passwordHash: simpleHash(password) } : p
        )
      );
    },
    [setPeople]
  ...
  const checkPassword = useCallback(
    (personId: string, password: string): boolean => {
      return verifyPassword(personId, password, people);
    },
    [people]
  );
  ...
  const registerFromSync = useCallback(
    (name: string, code: string): { success: boolean; personName?: string } => {
      ...
      // Also sync to localStorage for backward compatibility
      storePasswordHash(newPersonId, defaultPassword);
      ...
      // Set current user
      setCurrentUserId(newPersonId);

      return { success: true, personName: name };
    },
    [people, tasks, studentProfiles, setPeople, setTasks, setStudentProfiles, setCurrentUserId]
  );
