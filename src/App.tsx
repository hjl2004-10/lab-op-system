...
import { CalendarDays, Cloud, CloudAlert, LoaderCircle, LogOut } from "lucide-react";
...
export default function App() {
  ...
  const state = useAppState(auth.user, autoSave);
  ...
  useEffect(() => {
    ...
    setNewTask((previous) => {
      if (previous.assigneeId && state.people.some((person) => person.id === previous.assigneeId)) {
        return previous;
      }
      return {
        ...previous,
        assigneeId: state.isAdmin
          ? state.people.find((person) => person.status === "active")?.id || ""
          : state.currentUserId || "",
      };
    });
  }, [state.currentUserId, state.isAdmin, state.people]);
        ...
        <>
          <header className="workspace-header">
            <div className="workspace-title">
              <span className="workspace-logo"><CalendarDays size={17} /></span>
              <div><strong>YANG11 LAB</strong><span>实验进度协作台</span></div>
            </div>
            <div className="workspace-session">
              <span className={`sync-indicator ${state.syncStatus}`}>
                {state.syncStatus === "error" ? <CloudAlert size={14} /> : <Cloud size={14} />}
                {state.syncStatus === "saving" ? "保存中" : state.syncStatus === "error" ? "保存失败" : "已同步"}
              </span>
              <span className="session-user">
                <strong>{auth.user.name}</strong>
                <small>{auth.user.role === "admin" ? "管理员" : "学生"}</small>
              </span>
              <button
                className="logout-button"
                onClick={async () => {
                  try { await state.flushSave(); } finally { await auth.logout(); }
                }}
                title="退出登录"
                aria-label="退出登录"
              >
