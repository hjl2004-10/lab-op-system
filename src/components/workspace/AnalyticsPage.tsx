...
export default function AnalyticsPage({
  ...
  return (
    <main className="min-w-0 space-y-4 bg-slate-50 p-4 text-slate-900 dark:bg-slate-950 dark:text-slate-100 sm:p-6">
      <header className="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-end sm:justify-between dark:border-slate-700">
        <div>
      ...
      <section className="grid grid-cols-2 gap-2 md:grid-cols-5" aria-label="关键指标">
        {kpis.map(({ label, value, icon: Icon }) => (
          ...
          >
            <span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300">
